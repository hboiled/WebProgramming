<?php
    ...
    $rows = count($topTen);
    $highestFreqMovie = $topTen[0]["count"];
    ...
    //vertical line
    imageline($img, ($colWidth - 40), 0, ($colWidth - 40), ($height - 34), $black);
    //horizontal line
    ...
    imagestring($img, 31, 0, $height - 30, "count", $black);

    // build graph
    for ($i = 0; $i < $rows; $i++) {
        $colHeight = ($height / 100) * (($topTen[$i]["count"] / $maxHeight) * 100);
        // add 1 to $i to get block passed labels
        $x1 = $colWidth * ($i + 1);
        $y1 = $height - $colHeight;
        $x2 = $colWidth * ($i + 2) - $padding;
        $y2 = $height - 35;
        $colour = $colourSet[$i];

        imagefilledrectangle($img, $x1, $y1, $x2, $y2, $colour);
        imagestring($img, 5, ($x1 + $colWidth/4), $y2 +5, $topTen[$i]["count"], $black);
    }

    // write movie names
    for ($int = 0; $int < $rows; $int++) {
        $colour = $colourSet[$int];
        $x = $colWidth + ($colWidth * $int);
        $y = 10 + ($int * 15);

        imagestring($img, 5, $x, $y, $topTen[$int]["title"], $colour);
    }
